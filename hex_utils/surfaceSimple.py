''' Created on 30 May 2016 @author: lads ''' import numpy as np from mpl_toolkits.mplot3d import Axes3D import matplotlib.pyplot as plt from matplotlib import cm 
from matplotlib.colors import LightSource import random def well(x, y, x0, y0, depth):
    return depth + ((x - x0)/300.0)**(2) + ((y - y0)/300.0)**(2) def fun(x, y):
    z = []
...
fig = plt.figure() ax = fig.add_subplot(111, projection='3d') x = y = np.arange(0, 2000, 2) X, Y = np.meshgrid(x, y) zs = np.array([fun(x,y) for x,y in 
zip(np.ravel(X), np.ravel(Y))]) Z = zs.reshape(X.shape) ls = LightSource(270, 45)
...
rgb = ls.shade(Z, cmap=cm.gist_earth, vert_exag=0.1, blend_mode='soft') ax.plot_surface(X, Y, Z, facecolors=rgb) ax.set_xlabel('X') ax.set_ylabel('Y') 
